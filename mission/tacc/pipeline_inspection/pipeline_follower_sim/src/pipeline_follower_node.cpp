...
PipelineFollowerNode::PipelineFollowerNode() : Node("pipeline_follower_node") {
    input_topic_lines_ =
        this->declare_parameter<std::string>("input_topic_lines");
    input_topic_info_ =
        this->declare_parameter<std::string>("input_topic_info");
    input_topic_pose_ =
        this->declare_parameter<std::string>("input_topic_pose");
    input_topic_altitude_ =
        this->declare_parameter<std::string>("input_topic_altitude");
    ...
    debug_waypoint_topic_ = this->declare_parameter<std::string>(
        "debug_waypoint_topic", "/debug/waypoint");
    debug_service_off_topic_ = this->declare_parameter<std::string>(
        "debug_service_off_topic", "/debug/send_waypoints_service_off");

    debug_wp_pub_ = this->create_publisher<vortex_msgs::msg::Waypoint>(
        debug_waypoint_topic_, 10);
    service_off_pub_ = this->create_publisher<std_msgs::msg::Bool>(
        debug_service_off_topic_, 10);

    sub_line = this->create_subscription<vortex_msgs::msg::LineSegment2DArray>(
        input_topic_lines_, rclcpp::SensorDataQoS(),
        std::bind(&PipelineFollowerNode::linesCb, this, std::placeholders::_1));

    sub_pose = this->create_subscription<nav_msgs::msg::Odometry>(
        input_topic_pose_, rclcpp::SensorDataQoS(),
    ...
    sub_altitude = this->create_subscription<vortex_msgs::msg::DVLAltitude>(
        input_topic_altitude_, rclcpp::SensorDataQoS(),
        std::bind(&PipelineFollowerNode::altitudeCb, this,
                  std::placeholders::_1));

    sub_info = this->create_subscription<sensor_msgs::msg::CameraInfo>(
        input_topic_info_, rclcpp::SensorDataQoS(),
        std::bind(&PipelineFollowerNode::infoCb, this, std::placeholders::_1));

    client_ = this->create_client<vortex_msgs::srv::SendWaypoints>(
        "/nautilus/waypoint_addition");
        ...
        [this](const std_srvs::srv::Trigger::Request::SharedPtr,
            ...
            if (goal_blocked_) {
                RCLCPP_INFO(this->get_logger(),
                            "Goal reached — blocking execution");
            } else {
                RCLCPP_INFO(this->get_logger(),
                            "Goal cleared — resuming execution");
            }
...
void PipelineFollowerNode::linesCb(
    const vortex_msgs::msg::LineSegment2DArray::SharedPtr msg) {
    latest_lines_ = *msg;
    have_lines_ = true;
}

void PipelineFollowerNode::poseCb(
    ...
    robot_yaw_ = yaw;
    RCLCPP_WARN_THROTTLE(this->get_logger(), *get_clock(), 2000, "yaw = %f",
                         robot_yaw_);
    have_pose_ = true;
}

void PipelineFollowerNode::altitudeCb(
    const vortex_msgs::msg::DVLAltitude::SharedPtr msg) {
    robot_a_ = msg->altitude;
}

void PipelineFollowerNode::infoCb(
    const sensor_msgs::msg::CameraInfo::SharedPtr msg) {
    K_ = cv::Matx33d(msg->k[0], msg->k[1], msg->k[2], msg->k[3], msg->k[4],
...
void PipelineFollowerNode::timerTick() {
    RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000,
                         "tick: have_lines=%d have_pose=%d", have_lines_,
                         have_pose_);

    if (!have_lines_ || !have_pose_) {
        return;
    }
...
void PipelineFollowerNode::sendOrDebugWaypoint(double x,
                                               double y,
                                               double z,
                                               double yaw,
        ...
        if (dist < MIN_WP_DIST) {
            ...
            if (min_dist_skip_count_ < MAX_MIN_DIST_SKIPS) {
                RCLCPP_DEBUG_THROTTLE(
                    ...
                    "Waypoint skipped (distance %.3f m < %.2f m) (%d/%d)", dist,
                    MIN_WP_DIST, min_dist_skip_count_, MAX_MIN_DIST_SKIPS);
                return;
            }
            RCLCPP_WARN(
                get_logger(),
    ...
    // Hold target altitude on translating waypoints, but not on
    // orientation-only ones -- commanding altitude there can destabilise the
    // heading convergence.
    wp.keep_altitude =
    ...
    client_->async_send_request(req, [this,
                                      wp](rclcpp::Client<
                                          vortex_msgs::srv::SendWaypoints>::
                                              SharedFuture future) {
        try {
            auto resp = future.get();
            RCLCPP_WARN(this->get_logger(), "Sent waypoint: success=%d",
                        resp->success);
            ...
            RCLCPP_WARN(
                this->get_logger(),
                "Waypoint send failed (exception). Publishing debug waypoint.");
        }
    });
}

void PipelineFollowerNode::enqueueWaypoint(const vortex_msgs::msg::Waypoint& wp,
                                           bool overwrite_prior,
                                           bool take_priority,
                                           double switching_threshold) {
    debug_wp_pub_->publish(wp);
...
void PipelineFollowerNode::handleSingleLine(
    const vortex_msgs::msg::LineSegment2D& line) {
    RCLCPP_WARN_THROTTLE(this->get_logger(), *get_clock(), 2000,
                         "starting single line handling");
    ...
    auto [xc, yc] =
        rotateXY(camera_placment_x_, camera_placment_y_, robot_yaw_);

    double x_in_meters = robot_x_ + dx + xc;
    double y_in_meters = robot_y_ + dy + yc;

    if (abs(p1.x - p2.x) >= image_width / 7) {
        double newYaw = angleBetweenLinesRad(
            robot_yaw_, p1, p2, cv::Point2f(0, 0), cv::Point2f(1, 1), true);
        if (abs(abs(robot_yaw_) - abs(newYaw)) > 1.35) {
            sendOrDebugWaypoint(
                x_in_meters, y_in_meters, robot_z_, robot_yaw_, true, false,
                vortex_msgs::msg::WaypointMode::FORWARD_HEADING, 0.3);
            return;
        }
        sendOrDebugWaypoint(robot_x_, robot_y_, robot_z_, newYaw, true, false,
...
void PipelineFollowerNode::handleTwoLines(
    ...
    if (!findLineIntersection(p1, p2, q1, q2, cross)) {
        RCLCPP_WARN(this->get_logger(), "didn't find intersection");
        return;
    }
    ...
    if (cross_history_.size() == 3) {
        ...
        if (avg_dist < 80.0 & cross.y < 300) {
            auto point_opt = groundDistanceFromPixel(
                cv::Point2d(cross.x, cross.y), K_, robot_a_);
            auto ray = K_.inv() * cv::Vec3d(cross.x, cross.y, 1.0);
            if (!point_opt) {
                RCLCPP_WARN(
                    this->get_logger(),
                    "did not hav point_opt ray.y = %.6f, pixel y = %.1f",
                    ray[1], cross.y);
                return;
            }
    ...
    RCLCPP_WARN(this->get_logger(),
                "did not have a consistent corner, dooing single line");
    handleSingleLine(l1);
}
...
float PipelineFollowerNode::pointSegmentDistance(const cv::Point2f& p,
                                                 const cv::Point2f& a,
                                                 const cv::Point2f& b) {
    cv::Point2f ab = b - a;
...
bool PipelineFollowerNode::findLineIntersection(const cv::Point2f& p1,
                                                const cv::Point2f& p2,
                                                const cv::Point2f& q1,
                                                const cv::Point2f& q2,
                                                cv::Point2f& out) {
    ...
    RCLCPP_WARN(this->get_logger(), "intersection = (%.1f, %.1f)",
