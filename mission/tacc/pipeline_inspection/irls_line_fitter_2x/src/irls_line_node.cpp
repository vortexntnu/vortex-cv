...
IRLSLineNode::IRLSLineNode() : Node("irls_line_node") {
    ...
    input_topic_info_ = declare_parameter<std::string>(
        "input_topic_info", "/pipeline/camera/camera_info");
    output_topic_img_ =
        declare_parameter<std::string>("output_topic_img", "/irls_line/image");
    output_topic_lines_ = declare_parameter<std::string>("output_topic_lines",
                                                         "/irls_line/lines");
    ...
    publish_original_if_fail_ =
        declare_parameter<bool>("publish_original_if_fail", true);
    ...
    // Acceptance
    min_segment_length_px_ =
        declare_parameter<double>("min_segment_length_px", 0.0);
    max_opposite_white_ratio_ =
        declare_parameter<double>("max_opposite_white_ratio", 0.4);
    ...
    min_segment_length_px_second_ = declare_parameter<double>(
        "min_segment_length_px_second", min_segment_length_px_);
    ...
    pub_lines_ = create_publisher<vortex_msgs::msg::LineSegment2DArray>(
        output_topic_lines_, 10);
}
...
LineModel IRLSLineNode::fitIRLS(const std::vector<cv::Point2f>& pts) {
    ...
    for (int iter = 0; iter < max_iters_; ++iter) {
        ...
        for (size_t i = 0; i < pts.size(); ++i)
            resid[i] = perpendicularDistance(L, pts[i]);
        ...
        if (loss_ == RobustLoss::HUBER) {
            ...
            for (size_t i = 0; i < pts.size(); ++i) {
                ...
                wts[i] =
                    (float)((std::fabs(r) <= d) ? 1.0 : (d / std::fabs(r)));
            }
        } else {
            const double c = tukey_c_;
            for (size_t i = 0; i < pts.size(); ++i) {
                double u = resid[i] / scale / c;
                wts[i] = (std::fabs(u) < 1.0)
                             ? (float)std::pow(1.0 - u * u, 2.0)
                             : 0.f;
            }
        }
        ...
        if (dv < (float)eps_change_ && dp < (float)eps_change_)
            break;
    }
    return L;
}
...
void IRLSLineNode::imageCb(const sensor_msgs::msg::Image::ConstSharedPtr msg) {
    ...
    if ((int)nz.size() < min_pixels_) {
        if (publish_original_if_fail_)
            publishImage(cv_ptr->image, msg->header);
        RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000,
                             "Too few mask pixels: %zu", nz.size());
        return;
    }
    ...
    if (cv_ptr->image.channels() == 3)
        ...
        cv::cvtColor(cv_ptr->image, color, cv::COLOR_GRAY2BGR);
    const int w = color.cols, h = color.rows;

    cv::Point p1a(0, 0), p1b(0, 0);
    bool first_ok = false;

    if (clippedSegmentFromPts(L1, pts, w, h, clip_to_object_, clip_max_dist_px_,
                              p1a, p1b)) {
        const cv::Vec4i seg1(p1a.x, p1a.y, p1b.x, p1b.y);
        const double len1 =
            std::hypot((double)(p1b.x - p1a.x), (double)(p1b.y - p1a.y));
        first_ok = len1 >= min_segment_length_px_ &&
                   hasEnoughWhiteUnderLine(mask, seg1, 10, 0.60,
                                           max_opposite_white_ratio_);
        if (first_ok) {
    ...
    if (find_second_line_) {
        ...
        for (const auto& q : pts) {
            if (perpendicularDistance(L1, q) > (float)removal_band_px_)
                pts2.push_back(q);
        }
        ...
        if ((int)pts2.size() >= min_pixels_second_) {
            ...
            if (clippedSegmentFromPts(L2, pts2, w, h, clip_to_object_,
                                      clip_max_dist_px_, p2a, p2b)) {
                const cv::Vec4i seg2(p2a.x, p2a.y, p2b.x, p2b.y);
                const double len2 = std::hypot((double)(p2b.x - p2a.x),
                                               (double)(p2b.y - p2a.y));
                second_ok = len2 >= min_segment_length_px_second_ &&
                            hasEnoughWhiteUnderLine(mask, seg2, 10, 0.60,
                                                    max_opposite_white_ratio_);
                if (second_ok) {
...
// ----------------------------- Publishers -----------------------------------
void IRLSLineNode::publishImage(const cv::Mat& bgr,
                                const std_msgs::msg::Header& header) {
    auto out =
        cv_bridge::CvImage(header, sensor_msgs::image_encodings::BGR8, bgr)
            .toImageMsg();
    pub_img_->publish(*out);
}

vortex_msgs::msg::LineSegment2D IRLSLineNode::makeSeg(const cv::Point& a,
                                                      const cv::Point& b) {
