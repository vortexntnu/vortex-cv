...
static inline float perpendicularDistance(const LineModel& L,
                                          const cv::Point2f& p) {
    float dx = p.x - L.x0;
    float dy = p.y - L.y0;
...
static inline LineModel weightedPCALine(const std::vector<cv::Point2f>& pts,
...
static inline std::vector<cv::Point2f> lineRectIntersections(const LineModel& L,
                                                             int w,
                                                             int h) {
    ...
    if (std::fabs(L.vx) > 1e-6f) {
        float t = (0.f - L.x0) / L.vx;
        add_if_inside(0.f, L.y0 + t * L.vy);
        t = (float(w - 1) - L.x0) / L.vx;
        add_if_inside((float)(w - 1), L.y0 + t * L.vy);
    }
    if (std::fabs(L.vy) > 1e-6f) {
        float t = (0.f - L.y0) / L.vy;
        add_if_inside(L.x0 + t * L.vx, 0.f);
        t = (float(h - 1) - L.y0) / L.vy;
        add_if_inside(L.x0 + t * L.vx, (float)(h - 1));
    }
    return out;
}

// Compute a segment from the fitted line, optionally clipped to the inlier
// span.
static inline bool clippedSegmentFromPts(const LineModel& L,
    ...
    if (cand.size() < 2) {
        p1 = cv::Point(cvRound(L.x0 - L.vx * 1000.f),
                       cvRound(L.y0 - L.vy * 1000.f));
        p2 = cv::Point(cvRound(L.x0 + L.vx * 1000.f),
                       cvRound(L.y0 + L.vy * 1000.f));
        return true;
    }
...
static inline bool intersectLines(const LineModel& A,
                                  const LineModel& B,
                                  cv::Point2f& out) {
...
static inline double whiteBandRatio(const cv::Mat& bin,
                                    const cv::Point2f& p1,
                                    const cv::Point2f& p2,
                                    const cv::Point2f& n,
                                    int bandHeight) {
    ...
    std::vector<cv::Point> poly = {clamp_pt(p1), clamp_pt(p2), clamp_pt(p2b),
                                   clamp_pt(p1b)};
...
// Check that:
//   (a) the band on the "under" side (downward) has >= minWhiteRatio white
//   pixels, AND (b) the band on the opposite side has < maxOppositeWhiteRatio
//   white pixels.
//
// Condition (b) rejects lines that cut through the object rather than along its
// edge — when both sides are white, the line is an interior cross-cut, not an
// edge.
static inline bool hasEnoughWhiteUnderLine(const cv::Mat& img,
                                           const cv::Vec4i& line,
                                           int bandHeight = 10,
