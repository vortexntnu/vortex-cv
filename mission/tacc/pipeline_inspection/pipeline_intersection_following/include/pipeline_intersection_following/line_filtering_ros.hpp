...
class LineFilteringNode : public rclcpp::Node {
   ...
   private:
    ...
    rclcpp::Subscription<sensor_msgs::msg::CameraInfo>::SharedPtr
        camera_info_sub_;
    rclcpp::Subscription<vortex_msgs::msg::DVLAltitude>::SharedPtr altitude_sub_;
    ...
    void line_callback(const vortex_msgs::msg::LineSegment2DArray::SharedPtr msg);
    ...
    bool find_intersection(const Eigen::Matrix<double, 2, 2>& line1,
                           ...
                           Eigen::Vector2d& intersection, double min_angle);
    ...
    vortex_msgs::msg::Waypoint make_waypoint(double x, double y, double yaw,
                                             uint8_t mode) const;
    ...
    double realign_yaw_threshold_ = 0.35; ///< Rotate-first if bearing error exceeds this (rad)

    // Resend throttling: avoid re-issuing a waypoint (which wipes the queue and
    // resets the reference filter) unless the target moved/rotated meaningfully.
    double min_wp_dist_ = 0.2;      ///< Min target move (m) before resending a translate wp
    double min_wp_yaw_ = 0.1;       ///< Min target yaw change (rad) before resending a rotate wp
    int max_resend_skips_ = 20;     ///< Force a resend after this many consecutive skips
