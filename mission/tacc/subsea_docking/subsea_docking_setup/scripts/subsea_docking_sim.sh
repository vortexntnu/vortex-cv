...
tmux send-keys -t $SESSION "cd $WS && source install/setup.bash && ros2 launch subsea_docking_setup front_camera_aruco.launch.py use_sim:=true board:=tacc" C-m

tmux split-window -h -t $SESSION
tmux send-keys -t $SESSION "cd $WS && source install/setup.bash && ros2 launch subsea_docking_setup down_camera_aruco.launch.py use_sim:=true board:=tacc" C-m
