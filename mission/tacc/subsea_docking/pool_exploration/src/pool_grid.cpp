...
PoolExplorationMap::PoolExplorationMap(double size_x,
                                       double size_y,
                                       double resolution,
...
void PoolExplorationMap::insert_line_in_grid(
    ...
    for (const auto& seg : segments) {
        int x0 = static_cast<int>((seg.p0.x - grid_.info.origin.position.x) /
                                  grid_.info.resolution);
        int y0 = static_cast<int>((seg.p0.y - grid_.info.origin.position.y) /
                                  grid_.info.resolution);
        int x1 = static_cast<int>((seg.p1.x - grid_.info.origin.position.x) /
                                  grid_.info.resolution);
        int y1 = static_cast<int>((seg.p1.y - grid_.info.origin.position.y) /
                                  grid_.info.resolution);
...
void PoolExplorationMap::bresenham_line_algoritm(int x0,
                                                 int y0,
                                                 int x1,
                                                 int y1) {
    ...
    if (!steep) {
        ...
        for (int i = 0; i <= dx; i++) {
            set_grid_cell(
                x0 + i, y,
                occupied_cell_);  // må declare occupied_cell_ i ros senere
...
void PoolExplorationMap::set_grid_cell(int x, int y, int value) {
    if (x < 0 || x >= static_cast<int>(grid_.info.width) || y < 0 ||
        y >= static_cast<int>(grid_.info.height)) {
        return;
    }
