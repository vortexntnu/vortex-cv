...
StandoffState::StandoffState(
    ...
    : ActionState(
          action_server_name,
          std::bind(&StandoffState::create_goal, this, std::placeholders::_1)),
      standoff_goal_(std::move(standoff_goal)) {}

valve_inspection_fsm::WaypointManagerAction::Goal StandoffState::create_goal(
    ...
    // Rotate the YAML offset (expressed in valve local frame) into odom frame.
    ...
    // Drone +X must align with -Z_valve so the drone faces the valve.
    ...
    const auto target_pose =
        vortex::utils::types::Pose::from_eigen(standoff_pos, q_drone);
