...
class PositionEstimator():
    def __init__(self):
        ...
        # self.focal_length = 2.12  # mm
        # self.max_width = 1280  # pxl
        # self.max_height = 720  # pxl
        ...
        self.fov_horizontal = 66.6  # Degrees
        self.fov_vertical = 52.4  # Degrees
    ...
    def main(self, bbox, depth):
        ...
        )  # Note no difference from double lense since fov and max pixels are both halved.
        ...
        redefined_angle_x = self.redefine_angles(angle_centre_object_x,
                                                 self.fov_horizontal)
        redefined_angle_y = self.redefine_angles(angle_centre_object_y,
                                                 self.fov_vertical)

        if self.use_single_lense:
            redefined_angle_x = self.redefine_angles(angle_centre_object_x,
                                                     self.fov_horizontal * 0.5)

        return length_x_mtr, length_y_mtr, redefined_angle_x, redefined_angle_y
