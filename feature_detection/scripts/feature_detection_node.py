...
class FeatureDetectionNode():
    ...
    def __init__(self, image_topic):
        ...
        self.zedSub = rospy.Subscriber(
            image_topic, Image, self.camera_callback)
        self.resetSub = rospy.Subscriber(
            '/feature_detection/reset',
            Empty,
            self.feature_detection_reset_callback)
        self.fsmStateSub = rospy.Subscriber(
            '/fsm/state', String, self.fsm_state_cb)
        ...
        self.i2rcpPub = rospy.Publisher(
            '/feature_detection/i2rcp_image', Image, queue_size=1)
        self.shapePub = rospy.Publisher(
            '/feature_detection/shapes_image', Image, queue_size=1)
        self.linesPub = rospy.Publisher(
            '/feature_detection/lines_image', Image, queue_size=1)
        self.pointAreasPub = rospy.Publisher(
            '/feature_detection/point_areas_image', Image, queue_size=1)
        self.BBoxPub = rospy.Publisher(
            '/feature_detection/bbox_image', Image, queue_size=1)
        ...
        self.timerPub = rospy.Publisher(
            '/feature_detection/fps_timer', Float32, queue_size=1)
                ...
                try:
                    ...
                    try:
                        bbox_points, bbox_area, points_in_rects, detection = self.feat_detection.classification(
                            self.cv_image, self.current_object, self.hsv_params, self.noise_rm_params)
                        pt_arr_msg = self.build_point_array_msg(
                            points_in_rects, self.current_object, self.image_shape[0], self.image_shape[1])
                    ...
                    self.cv_image_publisher(
                        self.hsvCheckPub, self.feat_detection.hsv_validation_img)
                    self.cv_image_publisher(
                        self.noiseRmPub,
                    ...
                    self.cv_image_publisher(
                        self.shapePub, self.feat_detection.rect_flt_img)
                    self.cv_image_publisher(
                        self.linesPub, self.feat_detection.line_fitting_img)
                    self.cv_image_publisher(
                        self.BBoxPub, self.feat_detection.bbox_img)
                    self.cv_image_publisher(
                        self.pointAreasPub, self.feat_detection.pointed_rects_img)
                    ...
                    timediff = (end - start)
                    # Take reciprocal of the timediff to get runs per second.
                    fps = 1 / timediff
