...
class FeatureDetectionNode():
    ...
    def __init__(self):
        ...
        self.zedSub = rospy.Subscriber(
            '/front/image_view/output', Image, self.camera_callback)
        ...
        self.BBoxPointsPub = rospy.Publisher(
            '/feature_detection/detection_bbox', BoundingBox, queue_size=1)
        ...
        while not rospy.is_shutdown():
            if self.cv_image is not None:
                try:
                    ...
                    hsv_img, hsv_mask, hsv_mask_validation_img = self.feat_detection.hsv_processor(
                        self.cv_image, *self.hsv_params)
                    self.cv_image_publisher(
                        self.hsvCheckPub,
                        hsv_mask_validation_img,
                        msg_encoding="bgr8")

                    nr_img = self.feat_detection.noise_removal_processor(
                        hsv_mask, *self.noise_rm_params)
                    self.cv_image_publisher(
                        self.noiseRmPub, nr_img, msg_encoding="mono8")
