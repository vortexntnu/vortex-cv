...
class CoreCVNode():
    ...
    def spin(self):
        while not rospy.is_shutdown():
            if self.cv_image is not None:
                try:
                    rospy.loginfo(self.min_max_params)
                    bgr_img = cv.cvtColor(self.cv_image, cv.COLOR_BGRA2BGR)
                    bgr_mask, bgr_filt_img = self.BGR_filter(bgr_img, tuple(self.min_max_params[0]), tuple(self.min_max_params[1]))                    
                    ...
                    self.cv_image_publisher(self.imgPub, bgr_filt_img, msg_encoding="bgr8")
    ...
    def dynam_reconfigure_callback(self, config):
        ...
        self.min_max_params[0][0] = config.test_min1
        self.min_max_params[0][1] = config.test_min2
        self.min_max_params[0][2] = config.test_min3
        self.min_max_params[1][0] = config.test_max1
        self.min_max_params[1][1] = config.test_max2
        self.min_max_params[1][2] = config.test_max3
