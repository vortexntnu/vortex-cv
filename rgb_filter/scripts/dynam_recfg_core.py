...
class DynamicReconfigure(object):
    def __init__(self, *args, **kwargs):        
        ...
        # Enum example
        enum_test = self.ddr.enum([ self.ddr.const("Small", "int", 3, "A small constant"),
                            self.ddr.const("Medium", "int", 5, "A medium constant"),
                            self.ddr.const("Large", "int", 7, "A large constant")],
                            "test")
        self.ddr.add_variable("test_enum", "A size parameter which is edited via an enum", 3, 3, 7, edit_method=enum_test)

        # Min Max slider examples
        self.ddr.add_variable("test_min1", "PLACEHOLDER",   0,    0,  255)
        self.ddr.add_variable("test_min2", "PLACEHOLDER",   0,    0,  255)
        self.ddr.add_variable("test_min3", "PLACEHOLDER",   0,    0,  255)
        self.ddr.add_variable("test_max1", "PLACEHOLDER",   255,    0,  255)
        self.ddr.add_variable("test_max2", "PLACEHOLDER",   255,    0,  255)
        self.ddr.add_variable("test_max3", "PLACEHOLDER",   255,    0,  255)
