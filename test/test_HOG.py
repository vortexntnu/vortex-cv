...
def threshold(hog_img):
    ...
    img_norm = cv2.normalize(hog_img, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8U)

    # Apply Otsu's method to calculate the threshold value
    ret, threshold = cv2.threshold(img_norm, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
