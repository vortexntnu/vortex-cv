    ...
    def fit(self, X, y):
        ...
        for _ in range(self.k): #while a == False or i > self.k:
            ...
            if inlier_ids.size > self.d:
                self.inlier_points = np.hstack([maybe_inliers, inlier_ids])
                better_model = copy(self.model).fit(X[self.inlier_points], y[self.inlier_points])

                this_error = self.metric(
                    y[self.inlier_points], better_model.predict(X[self.inlier_points])
                )
