...
class Tracker:
    ...
    def __init__(self):
        ...
        self.pub = rospy.Publisher("/tracking/mul_tracked_cv_objects",
                                   OdometryArray,
                                   queue_size=10)
        ...
        rospy.set_param("/reconfigure_server/p_no_match",
                        config_loaded["pdaf"]["p_no_match"])

        rospy.set_param("/reconfigure_server/Q_00",
                        config_loaded["pdaf"]["Q"][0][0])
        rospy.set_param("/reconfigure_server/Q_11",
                        config_loaded["pdaf"]["Q"][1][1])
        rospy.set_param("/reconfigure_server/Q_22",
                        config_loaded["pdaf"]["Q"][2][2])
        rospy.set_param("/reconfigure_server/Q_33",
                        config_loaded["pdaf"]["Q"][3][3])

        rospy.set_param("/reconfigure_server/R_00",
                        config_loaded["pdaf"]["R"][0][0])
        rospy.set_param("/reconfigure_server/R11",
                        config_loaded["pdaf"]["R"][1][1])

        rospy.set_param("/reconfigure_server/N_resurrect",
                        config_loaded["manager"]["N_resurrect"])
        rospy.set_param("/reconfigure_server/M_resurrect",
                        config_loaded["manager"]["M_resurrect"])
        rospy.set_param("/reconfigure_server/N_kill",
                        config_loaded["manager"]["N_kill"])
        rospy.set_param("/reconfigure_server/M_kill",
                        config_loaded["manager"]["M_kill"])
        rospy.set_param("/reconfigure_server/max_vel",
                        config_loaded["manager"]["max_vel"])
        rospy.set_param(
    ...
    def update_tuning_params(self):
        ...
        self.track_manager.N_kill = rospy.get_param(
            "/reconfigure_server/N_kill")
        self.track_manager.M_kill = rospy.get_param(
            "/reconfigure_server/M_kill")

        self.track_manager.max_vel = rospy.get_param(
            "/reconfigure_server/max_vel")
        ...
        for track in self.track_manager.tentative_tracks:
            track.pdaf.validation_gate_scaling_param = rospy.get_param(
                "/reconfigure_server/validation_gate_scaling_param")
            track.pdaf.p_no_match = rospy.get_param(
                "/reconfigure_server/p_no_match")
    ...
    def pack_odometry_msg(self, track: PDAF2MN):
        ...
        # - - - -  position
        x = track.pdaf.posterior_state_estimate.mean.reshape(4, )
...
if __name__ == "__main__":
    try:
        ...
        reconf_client = dynamic_reconfigure.client.Client(
            "reconfigure_server",
            timeout=30,
            config_callback=tracker.reconfigure_cb)
        rospy.spin()
