...
class PDAFTester:
    ...
    def create_observations_for_one_timestep(self, x, y):
        ...
        if True:
            obs[-1, 0] = (x + np.random.randn(1) *
                          self.pdaf.measurment_noise.covariance[0, 0])
            obs[-1, 1] = (y + np.random.randn(1) *
                          self.pdaf.measurment_noise.covariance[1, 1])
    ...
    def create_observations_for_one_timestep_simple_version(self, x, y):
        ...
        obs = np.ndarray((n_obs, 2), dtype=float)
        for i in range(n_obs):
            obs[i, 0] = (x + np.random.randn(1) *
                         self.pdaf.measurment_noise.covariance[0, 0])
            obs[i, 1] = (y + np.random.randn(1) *
                         self.pdaf.measurment_noise.covariance[1, 1])
