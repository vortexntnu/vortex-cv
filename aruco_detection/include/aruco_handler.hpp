...
public:
	 ...
	 */
	int detectMarkerPoses(cv::Mat &img, const cv::Ptr<cv::aruco::Dictionary> dictionary, std::vector<geometry_msgs::Pose> &poses, std::vector<int> &ids,
	                      double markerLength);
	 ...
	 * @param ids ids of the 4 markers. Added clockwise from the top-left corner.
	 * All markers have the default orientation
	 */
	cv::Ptr<cv::aruco::Board> createRectangularBoard(float markerSize, float xDist, float yDist, const cv::Ptr<cv::aruco::Dictionary> &dictionary,
...
protected:
	 ...
	 * @param aa rotation vector with rotation axis in the direction it points and
	 * angle "norm(aa)"
	 ...
	 * @param rvec rotation vector in axis-angle representation. This is what
	 * Open-cv uses
