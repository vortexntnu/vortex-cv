...
class ImagePreprocessing:
    ...
    def __init__(self, clahe_cliplim, clahe_tilesize):

        self.clahe = cv.createCLAHE(clipLimit=clahe_cliplim,
                                    tileGridSize=(clahe_tilesize,
                                                  clahe_tilesize))
    ...
    def gamma_correction(self,
        ...
        if ch != None:
            new_img = (img[:, :, 0] * alpha +
                       np.full_like(img[:, :, 0], beta)).astype(np.uint8)
    ...
    def gray_world(self, img):
        ...
        illum_red, illum_green, illum_blue = np.sum(img[:, :, 2]) / N, np.sum(
            img[:, :, 1]) / N, np.sum(img[:, :, 0]) / N
