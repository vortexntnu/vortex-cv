...
class CalibrationNode():
    ...
    def __init__(self):
        ...
        self.criteria = (cv.TERM_CRITERIA_EPS + cv.TERM_CRITERIA_MAX_ITER, 30,
                         0.001)
        ...
        self.objp[:, :2] = np.mgrid[0:self.chessboardSize[0],
        ...
        self.zedLeftSub = rospy.Subscriber(left_img_topic, Image,
                                           self.camera_callback_left)
        self.zedRightSub = rospy.Subscriber(right_img_topic, Image,
                                            self.camera_callback_right)

        self.calibLeftPub = rospy.Publisher(
            "camera_calibration/rectified/left", Image, queue_size=10)
        self.calibRightPub = rospy.Publisher(
            "camera_calibration/rectified/right", Image, queue_size=10)
        ...
        self.first_image_left_msg = rospy.wait_for_message(
            left_img_topic, Image)
        self.first_image_right_msg = rospy.wait_for_message(
            right_img_topic, Image)

        try:
            self.cv_image_left = self.bridge.imgmsg_to_cv2(
                self.first_image_left_msg, "passthrough")
            self.image_shape_left = self.cv_image_left.shape
        except CvBridgeError:
            self.image_shape_left = (720, 1280, 4)

        try:
            self.cv_image_right = self.bridge.imgmsg_to_cv2(
                self.first_image_right_msg, "passthrough")
    ...
    def calibrate(self):
        ...
        retL, cornersL = cv.findChessboardCorners(grayL, self.chessboardSize,
                                                  None)
        retR, cornersR = cv.findChessboardCorners(grayR, self.chessboardSize,
                                                  None)
        ...
        if self.image_counter == 50:
            ...
            retL, cameraMatrixL, distL, rvecsL, tvecsL = cv.calibrateCamera(
                self.objpoints, self.imgpointsL, frameSize, None, None)
            ...
            retR, cameraMatrixR, distR, rvecsR, tvecsR = cv.calibrateCamera(
                self.objpoints, self.imgpointsR, frameSize, None, None)
            ...
            self.update_config(newCameraMatrixL, distL, newCameraMatrixR,
                               distR, str((frameSize[1], frameSize[0])))

        #self.calibPub.publish(left_calib_string)

    def update_config(self, newCameraMatrixL, distL, newCameraMatrixR, distR,
                      resolution):
        """
        ...
        try:
            ...
            rospy.loginfo("Config file has been saved in: " +
    ...
    def rectify_image(self):
        ...
        rect_image_right = cv.undistort(imgR, self.newCameraMatrixR,
                                        self.distR)
