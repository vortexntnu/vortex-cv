...
void UDFCWrapperNode::getCameraMatrix() {
  ...
  cv::Matx33f Matrix(fx, 0, cx, 0, fy, cy, 0, 0, 1);

  CameraMatrix = Matrix;
}

void UDFCWrapperNode::getDistortionCoefficents() {
  // This function turns the calibration parameters into a vector of distortion
  // params.
...
UDFCWrapperNode::UDFCWrapperNode(ros::NodeHandle nh) {
  ...
  for (int i = 0; i < paramNames.size(); i++) {
    nh.getParam(paramNames[i],
                calibrationParams[i]); // Getting calibration params
  }
  ...
  ros_image_raw_publisher =
      nh.advertise<sensor_msgs::Image>(image_raw_topic, 10);
  ros_image_rect_publisher =
      nh.advertise<sensor_msgs::Image>(image_rect_topic, 10);
  camera_frame = "udfc";
  getCVImage();
}
...
void UDFCWrapperNode::getCVImage() {
  if (_pipeline_id == 0) {
    image_pipeline = image_pipeline_prefix + std::to_string(_camera_id) +
                     " ! video/x-raw, format=YUY2 ! videoconvert ! "
                     "video/x-raw, format=BGR ! appsink drop=1";
  } else if (_pipeline_id == 1) {
    image_pipeline =
        image_pipeline_prefix + std::to_string(_camera_id) +
        " ! video/x-raw, format=YUY2 ! nvvidconv ! video/x-raw(memory:NVMM) ! "
        "nvvidconv ! video/x-raw, format=BGRx ! videoconvert ! video/x-raw, "
        "format=BGR ! appsink drop=1";
  } else if (_pipeline_id == 2) {
    image_pipeline = image_pipeline_prefix + std::to_string(_camera_id) +
                     " ! image/jpeg, format=MJPG ! nvv4l2decoder mjpeg=1 ! "
...
void UDFCWrapperNode::toImageRaw(cv::Mat cv_image) {
  // The function takes in a cv image and converts it into a ros image and
  // publishes it.
...
void UDFCWrapperNode::toImageRect(cv::Mat cv_image) {
  // The function takes in a cv image, rectifies it, and converts it into a ros
  ...
  if (!initFlag) {
    ...
    cv::getOptimalNewCameraMatrix(CameraMatrix, distortionCoefficents, s, 1, s1,
                                  rectPointer, false);

    initFlag = true;
  }
  cv::undistort(_cv_image, dst, CameraMatrix, distortionCoefficents);
  dst = dst(*rectPointer); // This crops the image according to the roi found
  ...
  header.frame_id = camera_frame;
  img_bridge =
      cv_bridge::CvImage(header, sensor_msgs::image_encodings::BGR8, dst);
