...
class PreprocessingNode():
    ...
    def __init__(self):
        ...
        # Confidence map
        rospy.Subscriber(camera_ns + '/confidence/confidence_map', Image,
                         self.confidence_cb)
        self.maskedMapImagePub = rospy.Publisher(
            '/cv/preprocessing/confidence_map_masked', Image, queue_size=1)

        # Depth Registered
        rospy.Subscriber(camera_ns + '/depth/depth_registered', Image,
        ...
        rospy.Subscriber(camera_ns + '/rgb/image_rect_color', Image,
        ...
        # Pointcloud
        rospy.Subscriber(camera_ns + '/point_cloud/cloud_registered',
        ...
        rospy.wait_for_message(camera_ns + '/point_cloud/cloud_registered',
                               PointCloud2)
    ...
    def ros_image_publisher(self,
                            publisher,
                            cv_image,
                            msg_encoding="passthrough"):
        """
    ...
    def spin(self):
        while not rospy.is_shutdown():
            ...
            masked_map, masked_as_cv_image = self.confMap.create_mask(
                self.cfd_cv, 3)
            # Bridge image data from cv_image to Image data
            self.ros_image_publisher(self.maskedMapImagePub,
                                     masked_as_cv_image)

            # Pointcloud masking
            confident_pointcloud = self.confMap.add_mask_to_pointcloud(
                masked_map, self.pointcloud_msg)
            ...
            confident_depth = self.confMap.add_mask_to_cv_image(
                masked_map, self.dpth_cv)
            self.ros_image_publisher(self.confident_depthPub, confident_depth)

            # RGB masking
            masked_rgb = self.confMap.add_mask_to_cv_image(
                masked_map, self.rgb_cv)
            self.ros_image_publisher(self.confident_rectImagePub, masked_rgb,
                                     "bgra8")
