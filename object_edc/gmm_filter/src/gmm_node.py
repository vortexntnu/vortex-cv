...
class EKFNode:
    ...
    def __init__(self):
        ...
        self.n = np.random.normal(
            0, 0.2**2, 3
        )  #Noise added TODO is there a better way to disturbe the filter (Ivan)
        ...
        )  # TODO Add a tf buffer length? tf2_ros.Buffer(rospy.Duration(1200.0)) (Kristian)
    ...
    def ekf_function(self, pw_wc, Rot_wc, Rot_cl, z):
        ...
        measurement_model = NED_linear_landmark(self.sigma_z, pw_wc, Rot_wc,
                                                Rot_cl, self.H_lam)
        ...
        gauss_x_pred, gauss_z_pred, gauss_est = my_ekf.step_with_info(
    ...
    def transformbroadcast_camera_gate(self, parent_frame, child_frame,
                                       msg_position, msg_pose):
        t = TransformStamped()
        t.header.stamp = rospy.Time.now()
        t.header.frame_id = parent_frame
    ...
    def obj_pose_callback(self, msg):
        ...
        if self.using_global_fake_object == 1:
            tf_lookup_cg = self.__tfBuffer.lookup_transform(
                self.child_frame, self.object_frame, rospy.Time(),
                rospy.Duration(2))

            obj_pose_position_c = np.array([
                tf_lookup_cg.transform.translation.x,
                tf_lookup_cg.transform.translation.y,
                tf_lookup_cg.transform.translation.z
            ])

            tf_lookup_wg = self.__tfBuffer.lookup_transform(
                self.parent_frame, self.object_frame, rospy.Time(),
                rospy.Duration(2))
            obj_pose_pose_c = np.array([
                tf_lookup_cg.transform.rotation.x,
        ...
        self.transformbroadcast_camera_gate(self.child_frame, "gate_detected",
                                            obj_pose_position_c,
                                            obj_pose_pose_c)
        tf_lookup_wg = self.__tfBuffer.lookup_transform(
            self.parent_frame, "object_gate_detected", rospy.Time(),
            rospy.Duration(2))

        obj_pose_position_wg = np.array([
            tf_lookup_wg.transform.translation.x,
            tf_lookup_wg.transform.translation.y,
            tf_lookup_wg.transform.translation.z
        ])

        obj_pose_pose_wg = np.array([
            tf_lookup_wg.transform.rotation.x,
        ...
        Rot_wc = tft.quaternion_matrix([
            tf_lookup_wc.transform.rotation.x,
        ...
        z_phi, z_theta, z_psi = tft.euler_from_quaternion(obj_pose_pose_wg,
                                                          axes='sxyz')
        ...
        #Data from EKF
        gauss_x_pred, gauss_z_pred, gauss_est = self.ekf_function(
            pw_wc, Rot_wc, Rot_cl, z)
        ...
        self.publish_gate(msg.header.frame_id, ekf_position,
                          ekf_pose_quaterion)
    ...
    def gate_hypotheses(self, z):
        ...
        for i in range(N):
            ...
            mahalanobis_distance = np.matmul(
                error.T, nla.solve(self.active_filters[0].cov + self.R, error))
    ...
    def gmm_callback(self, msg):
        ...
        z_phi, z_theta, z_psi = tft.euler_from_quaternion(
            measurement_orientation, axes="sxyz")
        z_eulers = np.array([z_phi, z_theta, z_psi])

        z = np.append(measurement_position, z_eulers)
        z_gauss = MultiVarGaussian(z, self.R)

        gated_inds, m_distences = self.gate_hypotheses(self.active_hypotheses,
                                                       z)

        self.active_hypotheses, ass_ind = self.hypotheses_update(
            gated_inds, m_distences)
