...
class PointCloudMapping():
    ...
    def object_orientation_from_xy_area(
            self, area_with_limits, pointcloud_data):
        """
        ...
        for x in range(xmin - 1, xmax - 1):
            for y in range(ymin - 1, ymax - 1):
                pt_gen = point_cloud2.read_points(
                    pointcloud_data, skip_nans=True, uvs=[[x, y]])
                for pt in pt_gen:
    ...
    def object_position_from_xy_point(self, x_pixel, y_pixel, pointcloud_data):
        ...
        if is_pointcloud:
            ...
            pt_gen = point_cloud2.read_points(
                pointcloud_data, skip_nans=False, uvs=[[x_pixel, y_pixel]])
            for pt in pt_gen:
    ...
    def object_orientation_from_point_list(self, point_list, pointcloud_data):
        ...
        for point in point_list:
            pt_gen = point_cloud2.read_points(
                pointcloud_data, skip_nans=True, uvs=[[point[0], point[1]]])
            for pt in pt_gen:
