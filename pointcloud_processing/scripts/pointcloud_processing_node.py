...
class PointcloudProcessingNode():
    ...
    def feat_det_cb(self, msg):
        ...
        for point in msg.point_array:
            point_list.append(
                (point.y, point.x)
            )  # TODO: change back from y, x to x, y as it is supposed to be the latter
        ...
        self.send_position_orientation_data(headerdata, positiondata,
                                            orientationdata, objectID)
    ...
    def pointcloud_camera_cb(self, msg_data):
        ...
        """
        assert isinstance(
            msg_data, PointCloud2)  # This may be the wrong place to put this
    ...
    def send_position_orientation_data(self, headerdata, positiondata,
                                       orientationdata, name):
        """
        ...
        """
        if orientationdata:
            self.send_pose_message(headerdata, positiondata, orientationdata,
                                   name)
            self.send_ObjectPosition_message(headerdata, positiondata,
                                             orientationdata, name)
    ...
    def send_pose_message(self, headerdata, position_data, quaternion_data,
        ...
        """
        posePub = rospy.Publisher('/pointcloud_processing/object_pose_rviz/' +
                                  name,
                                  PoseStamped,
                                  queue_size=1)
        p_msg = PoseStamped()
    ...
    def send_ObjectPosition_message(self, headerdata, position_data,
                                    quaternion_data, name):
        """
        Publishes a PoseStamped as a topic under /pointcloud_processing/object_pose
        ...
        Returns:
            Topic:
                /pointcloud_processing/object_pose/name where name is your input
        """
        objposePub = rospy.Publisher('/pointcloud_processing/object_pose/' +
                                     name,
                                     ObjectPosition,
                                     queue_size=1)
