...
def test_gate_hypotheses(active_hypotheses, active_hypotheses_count,
                         gmf_weights):
    ...
    ref_weights = gmf_weights[
        2]  # Hypothesis 2 is the only gated one, giving the third index in weights
...
def test_associate_and_update(gated_hypotheses, gated_weights, gated_inds,
                              gmf_weights, active_hypotheses):
    #### PARAMETERS ####
    boost_prob = 0.1
    init_weight = 0.2
    #### GIVE SOME INPUTS ####
...
def test_gmf_eval():
    ...
    active_hypotheses = [
        MultiVarGaussian(0, 5) for i in range(active_hypotheses_count)
    ]
...
active_hypotheses = [
    MultiVarGaussian(means[i], hyp_cov) for i in range(len(means))
]
active_hypotheses_count = len(active_hypotheses)
predicted_zs = means
