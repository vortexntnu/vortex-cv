...
class PathFollowingNode():
    ...
    def __init__(self):
        ...
        # Publishers
        self.udfcPub = rospy.Publisher("/path_following/udfc",
                                       Image,
                                       queue_size=1)
        self.hsvPub = rospy.Publisher("/path_following/hsv_monkey",
                                      Image,
                                      queue_size=1)
        ...
        self.noise_filteredPub = rospy.Publisher(
            "/path_following/noise_filtered", Image, queue_size=1)
        ...
        self.feature_detector = feature_detection.FeatureDetection(
            self.image_shape)
    ...
    def path_calculations(self, img):
        ...
        img_drawn = cv2.circle(img, (cx, cy),
                               radius=1,
                               color=(0, 255, 0),
                               thickness=3)
    ...
    def batch_estimate_waypoint(self, t_udfc_odom):
        ...
        vx, vy, x0, y0 = cv2.fitLine(self.batch_line_params[:, :2],
                                     cv2.DIST_L2, 0, 0.1, 0.1)
        ...
        errors = [
            np.linalg.norm(uppestest_y_coord - points_img[i][:2])
    ...
    def path_following_udfc_cb(self, img_msg):
        ...
        _, hsv_mask, hsv_val_img = self.feature_detector.hsv_processor(
            udfc_img, *self.hsv_params)
        self.cv_image_publisher(self.hsvPub, hsv_val_img, msg_encoding="bgr8")

        noise_filtered_img = self.feature_detector.noise_removal_processor(
            hsv_mask, *self.noise_rm_params)
        self.cv_image_publisher(self.noise_filteredPub,
                                noise_filtered_img,
                                msg_encoding="8UC1")

        self.path_contour = self.feature_detector.contour_processing(
            noise_filtered_img,
            contour_area_threshold=3000,
            pfps=True,
            coloured_img=udfc_img,
            return_image=False)
        try:
            self.path_contour[:, 0]
        except:
        ...
        path_area, img_drawn, self.path_centroid = self.path_calculations(
            udfc_img)
        ...
        path_centroid_cam = np.matmul(self.K_opt_inv,
                                      np.append(self.path_centroid, 1))

        dp_ref = self.map_to_odom(path_centroid_cam[:2],
                                  t_udfc_odom,
                                  dp_ref=True)
        ...
        p_line, p0, img_drawn = self.find_contour_line(upper_contour_image,
                                                       img_drawn)
        ...
        if self.current_state == "path_execute":
            uppest_y_ind = np.argmin(upper_contour_image[:, 1])
            self.uppest_y_coords.append(upper_contour_image[uppest_y_ind, :])
            self.batch_line_params = np.vstack(
                (self.batch_line_params, np.vstack((p_line_odom, p0_odom))))
